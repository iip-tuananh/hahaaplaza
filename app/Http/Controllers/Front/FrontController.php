...
class FrontController extends Controller
{
    ...
    public function homePage() {
        ...
        $data['listProducts'] = Product::with(['image'])->where('status', 1)->limit(16)->inRandomOrder()->get();
        ...
        $data['productCategories'] = Category::query()->where(['type' => 1, 'parent_id' => 0])->where('show_home_page', 1)->orderBy('sort_order')->get();
    ...
    public function loadProductHomePage(Request $request)
    {
        if($request->filter == 'all') {
            ...
            ])->where('status', 1)->limit(16)->inRandomOrder()->get();
        } else {
            $category = Category::findBySlug($request->filter);
            ...
            ])->where('status', 1)->whereIn('cate_id', $arr_category_id)->limit(16)->inRandomOrder()->get();
        }
        $html = view('site.products.product_tab_home', compact('products'))->render();
    ...
    public function showProductDetail($slug) {
        try {
            ...
            return view('site.products.product_detail', compact('categories', 'product', 'productsRelated', 'category', 'arr_product_rate_images', 'bestSellerProducts', 'productTags'));
